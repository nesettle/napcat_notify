...
cd /d "%ROOT%"
"%PYTHON%" "%ROOT%compare_jinshuju.py" --entries-url "%ENTRIES_URL%" --qualification-file "%QUALIFICATION_FILE%" --profile-dir "%PROFILE_DIR%" --headless %*
...
if not "%EXIT_CODE%"=="0" (
  echo.
  echo compare_jinshuju.py exited with code %EXIT_CODE%.
)
